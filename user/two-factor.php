...
<head>
    ...
    <title>AtomPay | Enable 2FA</title>
    ...
    <style>
        ...
        * {
        ...
        .otp-input-group {
            display: flex;
            gap: 10px;
            justify-content: center; /* Center the OTP inputs */
        }

        .otp-input-group input {
            width: 50px; /* Fixed width for OTP boxes */
            height: 60px;
            text-align: center;
            font-size: 1.8rem;
            font-weight: 700;
...
<body>
    ...
    <main class="main-content">
        <div class="two-fa-card">
            ...
            <p class="subtitle">Secure your account with an extra layer of protection.</p>

            <form action="#" method="POST">
                <div class="form-group">
                    <label for="phoneNumber">Phone Number for Verification</label>
                    <input type="tel" id="phoneNumber" name="phoneNumber" placeholder="e.g., 08012345678" pattern="[0-9]{11}" maxlength="11" required>
                    <small class="text-muted">An OTP will be sent to this number for verification.</small>
                </div>

                <button type="submit" class="btn btn-primary" id="sendOtpBtn">Send Verification Code</button>

                <!-- OTP Verification Section (Initially hidden) -->
                <div class="form-group" id="otpSection" style="display: none; margin-top: 30px;">
                    <label for="otpCode">Enter Verification Code</label>
                    <div class="otp-input-group">
                        <input type="text" id="otp1" maxlength="1" pattern="[0-9]" inputmode="numeric" autocomplete="one-time-code">
                    ...
                    <span class="resend-otp">Didn't receive code? <a href="#">Resend Code</a></span>
                </div>
                
                <button type="submit" class="btn btn-primary" id="verifyOtpBtn" style="display: none;">Verify & Enable 2FA</button>
            </form>
        </div>
    </main>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            sendOtpBtn.addEventListener('click', (event) => {
                ...
                if (phoneNumberInput.checkValidity()) {
                    alert(`Sending OTP to ${phoneNumberInput.value}.`);
                    ...
                    phoneNumberInput.reportValidity(); // Show browser validation message
                }
            });

            // Auto-focus to next OTP input and handle backspace
            ...
            verifyOtpBtn.addEventListener('click', (event) => {
                ...
                if (otp.length === 6 && /^\d+$/.test(otp)) {
                    ...
                    alert(`Verifying OTP: ${otp}. 2FA enabled!`);
